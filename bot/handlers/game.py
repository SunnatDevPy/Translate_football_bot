...
from db import Phone, About, Network
...
async def leagues_handler(call: CallbackQuery):
...
async def league_handler(call: CallbackQuery, state: FSMContext):
    data = call.data.split('_')
...
async def leagues_handler(call: CallbackQuery, bot: Bot):
    ...
    if data[-1] == 'home':
        await call.message.delete()
        if call.from_user.id in [1353080275, 5649321700]:
